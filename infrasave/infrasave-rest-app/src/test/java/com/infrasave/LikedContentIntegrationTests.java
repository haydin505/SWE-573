...
public class LikedContentIntegrationTests {
  ...
  @Test
  @Order(3)
  public void shouldAddToLikedContent() throws Exception {
    MockHttpSession session = getMockHttpSession();
    AddMyContentRequest req = new AddMyContentRequest(1L);
    String content = MAPPER.writeValueAsString(req);
    mockMvc.perform(post("/my-content")
                        .contentType(MediaType.APPLICATION_JSON).session(session)
                        .content(content))
           .andExpect(status().isOk())
           .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
  ...
  @Test
  @Order(4)
  public void shouldGetLikedContents() throws Exception {
    MockHttpSession session = getMockHttpSession();
    mockMvc.perform(get("/my-content")
                        .contentType(MediaType.APPLICATION_JSON).session(session))
           .andExpect(status().isOk())
           .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
           .andExpect(jsonPath("successful", is(true)))
           .andExpect(content().json("{\"successful\":true,\"errorCode\":null,\"errorTitle\":null,"
                                     ...
                                     + "\"title\":\"Test-title\",\"url\":\"test-url\",\"imageUrl\":\"test-imageUrl\","
                                     + "\"description\":\"Test-description\",\"myContent\":true,"
                                     ...
                                     + "\"friendCount\":null,\"friends\":null,\"friendDTO\":null},\"tags\":[]}]}"));
  }
  ...
  public void shouldDeleteContent() throws Exception {
    MockHttpSession session = getMockHttpSession();
    mockMvc.perform(delete("/my-content/" + "1")
                        .contentType(MediaType.APPLICATION_JSON).session(session))
           .andExpect(status().isOk())
           .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
           .andExpect(jsonPath("successful", is(true)));
