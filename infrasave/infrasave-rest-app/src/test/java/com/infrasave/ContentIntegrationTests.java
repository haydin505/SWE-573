...
public class ContentIntegrationTests {
  ...
  @Test
  @Order(2)
  public void shouldGetMyFeed() throws Exception {
  ...
  public void shouldUpdateContent() throws Exception {
    MockHttpSession session = getMockHttpSession();
    ModifyContentRequest req =
        new ModifyContentRequest(1L, VisibilityLevel.EVERYONE, "Test-tit", "test-url", "test-imageUrl",
                                 "Test-description",
                                 null);
    String content = MAPPER.writeValueAsString(req);
    mockMvc.perform(put("/contents")
                        .contentType(MediaType.APPLICATION_JSON).session(session)
                        .content(content))
           .andExpect(status().isOk())
           .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
